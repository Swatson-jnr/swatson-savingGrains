...
export default function PickupTable({ tableDetails }: TableProps) {
  ...
  useEffect(() => {
    ...
    if (paymentFilter.length > 0) {
      result = result.filter((row) => paymentFilter.includes(row.transferType));
    }
  ...
  }, [roleFilter, paymentFilter, dateRange, tableDetails]);
